...
export class ExpoRouterReactNavigationCheck implements DoctorCheck {
  ...
  async runAsync({ pkg }: DoctorCheckParams): Promise<DoctorCheckResult> {
    ...
    return {
      ...
      // TODO(@ubax): Replace [MIGRATION GUIDE URL] with the published migration guide URL.
      advice: [
        `Remove these packages from your package.json and replace any direct \`@react-navigation/*\` imports in your code with their expo-router equivalents. See [MIGRATION GUIDE URL] for more information.`,
      ],
    };
  }
}
